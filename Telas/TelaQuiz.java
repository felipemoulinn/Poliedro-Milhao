...
public class TelaQuiz extends JFrame {
    ...
    private void carregarPerguntasDoBanco() {
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASS)) {
            ...
            if (bancoQuestoes.isEmpty()) {
                JOptionPane.showMessageDialog(this, "Nenhuma pergunta encontrada no banco de dados!", "Erro", JOptionPane.ERROR_MESSAGE);
                criarBancoQuestoesPadrao();
            }
        } catch (SQLException e) {
            e.printStackTrace();
            JOptionPane.showMessageDialog(this, "Erro ao carregar perguntas: " + e.getMessage(), "Erro", JOptionPane.ERROR_MESSAGE);
            criarBancoQuestoesPadrao();
        }
    ...
    private void verificarResposta(JButton btnSelecionado) {
        ...
        if (respostaSelecionada.equals(respostaCorreta)) {
            ...
            Timer timer = new Timer(2000, e -> finalizarJogo(false));
            timer.setRepeats(false);
            timer.start();
        }
    }
    ...
    private void pularPergunta() {
        if (questaoAtual < bancoQuestoes.size() - 1) {
            ...
            JOptionPane.showMessageDialog(this, "Não há mais perguntas para pular!", "Aviso", JOptionPane.WARNING_MESSAGE);
        }
    }
    ...
    private void salvarPontuacao() {
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASS)) {
            ...
            e.printStackTrace();
            JOptionPane.showMessageDialog(this, "Erro ao salvar pontuação: " + e.getMessage(), "Erro", JOptionPane.ERROR_MESSAGE);
        }
    }
